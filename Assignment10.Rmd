...
PMT_walk <- skims %>%
  ...
  summarize(total_walk_miles_HBO = sum(PMT_walk_HBO),
            total_walk_miles = sum(PMT_walk_HBW),
            total_walk_miles = sum(PMT_walk_NHB))
...
PMT_car_HBO_alt <- skims_alt %>%
  ...
  summarize(total_SOV_miles_alt = sum(PMT_SOV_HBO_alt),
             total_HOV_miles_alt = sum(PMT_HOV_HBO_alt))

PMT_car_HBO_alt
```

```{r}
#using parameter for HBO trips for 2-plus carpool (daily) in NCHRP 716, Table 4-16
VMT_HBO_alt <- PMT_car_HBO_alt$total_SOV_miles_alt[1] + 
       PMT_car_HBO_alt$total_HOV_miles_alt[1] / 2.71
...
PMT_bike_HBO_alt <- skims_alt %>%
  ...
  summarize(total_bike_miles_alt = sum(PMT_bike_HBO_alt)
             )
